...
func Test_generateMultusAnnotationPatch(t *testing.T) {
	...
	assert.NoError(err)
	newPod, err := updatePodAnnotations(pod, vmi)
	assert.NoError(err)
	t.Log(newPod.Annotations[networkv1.NetworkStatusAnnot])
	assert.False(namescheme.PodHasOrdinalInterfaceName(services.NonDefaultMultusNetworksIndexedByIfaceName(newPod)))
}
